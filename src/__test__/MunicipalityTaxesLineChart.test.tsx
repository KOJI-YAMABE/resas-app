...
import type { MunicipalityTaxesResponse } from '../types/resas'

// 1) fetchMunicipalityTaxes をモック化
vi.mock('../apis/resasApi', () => ({
    fetchMunicipalityTaxes: vi.fn(),
}))
...
describe('MunicipalityTaxesLineChart', () => {
    ...
    it('都道府県が選択されていない場合、「都道府県を選択してください」と表示される', () => {
        ...
        // API は呼ばれないこと
        expect(fetchMunicipalityTaxes).not.toHaveBeenCalled()
    })

    it('都道府県が選択されている場合、API で地方税データを取得してグラフを表示する', async () => {
        // 3) ダミーの地方税データ
        const mockData: MunicipalityTaxesResponse[] = [
            { year: 2010, value: 1200 },
            { year: 2011, value: 1300 },
        ]
        // fetchMunicipalityTaxes が呼ばれたら、mockData を resolve
        ;(fetchMunicipalityTaxes as Mock).mockResolvedValueOnce(mockData)
        ...
        // 5) API が呼ばれるかチェック
        await waitFor(() => {
            expect(fetchMunicipalityTaxes).toHaveBeenCalledWith(13)
        })
